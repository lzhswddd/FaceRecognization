...
MTCNN::MTCNN(const string &model_path) {
	...
	enable = true;
	if (Pnet.load_param(param_files[0].data()) == -1)
		enable = false;
	if (Pnet.load_model(bin_files[0].data()) == -1)
		enable = false;
	if (Rnet.load_param(param_files[1].data()) == -1)
		enable = false;
	if (Rnet.load_model(bin_files[1].data()) == -1)
		enable = false;
	if (Onet.load_param(param_files[2].data()) == -1)
		enable = false;
	if (Onet.load_model(bin_files[2].data()) == -1)
		enable = false;
}
...
//Bbox MTCNN::detect(ncnn::Mat & img_)
//{
//	std::vector<Bbox> finalBbox_;
//	detect(img_, finalBbox_);
//	if (finalBbox_.empty()) {
//		fprintf(stderr, "error: Face Detection is empty!\n");
//		throw "error: Face Detection is empty!\n";
//	}
//	Bbox maxBbox = finalBbox_[0];
//	if (finalBbox_.size() != 1) {
//		for (std::vector<Bbox>::iterator iter = finalBbox_.begin() + 1; iter != finalBbox_.end(); ++iter)
//			if (maxBbox.score < iter->score)
//				maxBbox = *iter;
//	}
//	std::vector<Bbox>().swap(finalBbox_);
//	return maxBbox;
//}
